...
impl Pio {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn invert(&self) -> super::vals::Pio013Invert {
        let val = (self.0 >> 7usize) & 0x01;
        super::vals::Pio013Invert::from_bits(val as u8)
    }
    #[doc = "Input polarity."]
    #[inline(always)]
    pub const fn set_invert(&mut self, val: super::vals::Pio013Invert) {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn ecs(&self) -> super::vals::Pio013Ecs {
        let val = (self.0 >> 13usize) & 0x01;
        super::vals::Pio013Ecs::from_bits(val as u8)
    }
    #[doc = "Pull-up current source enable in I2C mode."]
    #[inline(always)]
    pub const fn set_ecs(&mut self, val: super::vals::Pio013Ecs) {
...
impl defmt::Format for Pio {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Pio {{ func: {:?}, mode: {:?}, slew: {:?}, invert: {:?}, digimode: {:?}, od: {:?}, ssel: {:?}, filteroff: {:?}, ecs: {:?}, egp: {:?}, i2cfilter: {:?} }}",
            self.func(),
            self.mode(),
            self.slew(),
            self.invert(),
